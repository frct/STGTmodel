...
def FMF(state, action, V, alpha, gamma, flupenthixol):
...
def MB(state, action, Q, T, R, alpha, gamma):
    r = SxA_R[state, action]
    ...
    return {'Advantage function': Advantage, 'Transition function': T, 'MB reward function' : R, 'Qvalues': Q}


def Integrate(state, omega, alpha, gamma, Q, T, R, V, flupenthixol):
    ...
    for index, action in enumerate(actions):
        ...
        newV = FMFResults['Value']
        #print(V)
        c = Feature(state, action)
        Advantage = MB(state, action, Q, T, R, alpha, gamma)['Advantage function']
        #print(Advantage)
        P[index] = (1 - omega) * Advantage + omega * newV[c]
        #print(P)
...
def ActionDistribution(state, omega, alpha, gamma, Q, T, R, V, beta, flupenthixol, injection):
    IntegrateResults = Integrate(state, omega, alpha, gamma, Q, T, R, V, flupenthixol)
    P = IntegrateResults['Integrated values']
    actions = IntegrateResults['Actions']
    n_possibleactions = len(P)
